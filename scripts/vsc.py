"""Check that the Cellveyor version in cellveyor/version.py matches pyproject.toml.

Uses Tree-sitter to parse cellveyor/version.py into a Concrete Syntax
Tree to extract the CELLVEYOR_VERSION constant, and uses the toml library
...
VERSION_VAR = "CELLVEYOR_VERSION"
VERSION_PATH = Path("cellveyor/version.py")
...
app = typer.Typer(
    name="version-check",
    help=(
        "Check that the CELLVEYOR_VERSION in cellveyor/version.py"
...
def _extract_version_from_main(main_path: Path, parser: Parser) -> str | None:
    """Return the string assigned to CELLVEYOR_VERSION in main_path, or None."""
...
def _update_version_file(main_path: Path, new_version: str) -> bool:
    """Update the CELLVEYOR_VERSION string in version.py. Return True if changed."""
...
@app.command()
def fix() -> None:
    """Update version.py so CELLVEYOR_VERSION matches the pyproject.toml version."""
    main_path = VERSION_PATH
    pyproject_path = Path("pyproject.toml")
    pyproject_version = _extract_version_from_pyproject(pyproject_path)
    if pyproject_version is None:
